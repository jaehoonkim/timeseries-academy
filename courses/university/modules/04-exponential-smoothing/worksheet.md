...
계속 옅어진다**는 뜻이다(α — 읽기는 '알파' — 가 0.3이면 오늘
무게 0.30, 어제 0.21,
...
- **변화량** — "하루에 얼마나 오르내리는 **중인가**"(기울기).
  홀트가 새로 끼운 부품이다. 갱신 비율은 β(읽기는 '베타').
...
배합 버전이라 보면 되고, 갱신 비율은 γ(읽기는 '감마')다.
...
**Q6.** 어디서 벌었고, 어디가 여전히 아픈가 — 읽는 법:

> `pd.DataFrame({"주간naive": …, "HW": …})`
> = 두 선수의 월별 |오차| 평균(U1 Q8에서 요일로 했던 그
> groupby)을 열 두 개짜리 **한 표에 나란히** 놓아라 — 달마다
> 누가 얼마나 벌었는지 한눈에 비교된다
>
> `e_hw.abs().nlargest(5)`
> = |오차|가 **가장 큰**(largest) 다섯 날을 날짜와 함께 뽑아라
> — 아픈 곳 탐지기다
...
답: ____________ (최악의 날들이 전부 공휴일이라는 것은, 남은
오차의 원인이 "그날이 빨간 날인가"라는 **달력 정보**라는 뜻이다
— 값의 과거만 보는 모형은 이것을 알 수 없다. 달력을 재료로
모형에 넣는 법은 U7에서 정면으로 다룬다)
...
**Q10.** 부품들에는 이름표가 하나 남았다 — **오차**(Error):
부품들이 설명하고 남는 몫(U2의 잔차가 모형 안에서 얻는 이름)으로,
모든 모형은 "관측 = 수준 (+추세) (+계절) + 오차"로 스스로를
쓴다. 이 가족의 정식 이름 **ETS**(Error·Trend·Seasonal)는 오차를
어떻게 넣는가(E)·추세(T)·계절(S)의 세 축으로 구성원을 분류하는
이름표다 — 수준은 모두가 가진 공통 부품이라 축이 되지 않는다.
두 데이터에 어떤 부품 조합이 최선이었는지 표로 정리하자:
...
(값의 출처: 지하철 행은 4절의 hw, 기온 행은 5절의 hw_t.) 채우고
나면 두 줄의 대비가 오늘의 결론이다 — 같은 모형인데 두 데이터가
고른 배합이 전혀 다르다. **부품과 배합은 데이터가 정한다** — U1
성격표의 모형 버전인 셈이다.
