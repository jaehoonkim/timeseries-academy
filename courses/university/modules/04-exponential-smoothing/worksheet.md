...
부품의 이름부터 정하자. SES가 배합으로 굴려 온 값 — "이 시계열이
**지금 어디쯤 있나**"의 매끄러운 현재값 — 을 **수준**(level)이라
부른다. 1절의 "오늘의 예측"이 바로 이것이다. 홀트는 여기에 두
번째 부품을 끼운다: **변화량** — "하루에 얼마나 오르내리는
중인가"(기울기) — 도 별도의 배합으로 함께 굴리는 것이다. 수준은
α로, 변화량은 β로 갱신하고, 예측은 "**수준 + 변화량**", 곧 지금
있는 곳에서 하루치만큼 더 간 곳이다. 예: 수준이 20도이고 요즘
하루 +0.3도씩 오르는 중이면 내일 예측은 20.3도 — naive(20도)가
못 하는 방향 반영이다. 코드는 옵션 하나 차이다. statsmodels의 이름표는
...
다른가**"의 보정값을 기억하는 부품으로, U1에서 만든 요일
프로필의 배합 버전이라 보면 된다. 예측은 "수준 + 변화량 + **내일
자리의 계절 보정**"이 되고, 보정 파형을 얼마나 빨리 갱신할지의
비율이 γ다. 예: 수준이 700만인데 내일이 일요일이고 일요일
보정이 -250만이면, 내일 예측은 450만.
