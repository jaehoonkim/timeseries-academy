...
통칭 **ETS**다. 수준·추세·계절이라는 **부품**을 하나씩 끼우며
조립한다.
...
즉 "**새 소식 α + 기억 (1 − α)**"의 배합. 오늘의 예측 안에는
어제의 배합이, 그 안에는 그제의 배합이 들어 있어서 — 과거가
멀수록 기하급수로 옅어지는 기억이 된다(이름의 '지수'가 여기서
온다).
...
훈련 2년이 스스로 고른 최선의 배합이 이것이다 — U1의 벽이 왜 그리
높았는지, U3의 "직접 정보는 어제뿐"이 왜 그 그림이었는지가 모형의
언어로 한 번에 설명된다.
...
Holt의 방법: 수준 배합에 **추세 부품**(오르내림의 방향을 기억하는
두 번째 배합, 비율 β)을 끼운 것. 코드는 옵션 하나 차이다:
...
답: ____________ (죽은 부품은 공짜가 아니다 — 성적을 갉아먹는다)

## 4. 계절 부품 — Holt-Winters, 그리고 첫 승리

계절 부품(주기적 파형을 기억하는 세 번째 배합, 비율 γ)까지 끼우면
**Holt-Winters**다. 지하철에 주기 7로 조립하자:
